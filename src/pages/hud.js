...
const handlers = {
  ...
  showFindMode(data) {
    ...
    inputElement.addEventListener("input", (executeQuery = function(event) {
      ...
      if (Utils.isFirefox() && event.isComposing) { return; }
  ...
  copyToClipboard(data) { return Utils.setTimeout(TIME_TO_WAIT_FOR_IPC_MESSAGES, function() {
  ...
  pasteFromClipboard() { return Utils.setTimeout(TIME_TO_WAIT_FOR_IPC_MESSAGES, function() {
    const focusedElement = document.activeElement;
  ...
  settings({ isFirefox }) { return Utils.isFirefox = () => isFirefox; }
};
