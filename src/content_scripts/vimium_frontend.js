...
var Frame = {
  ...
  init() {
    ...
    return window.addEventListener('unload', (forTrusted((event) => {
  ...
  return function () {
    if (highlightedFrameElement == null) {
      ...
      // Create a shadow DOM wrapping the frame so the page's styles don't interfere with ours.
        ...
        const _shadowDOM = (left = (left1 = (typeof highlightedFrameElement.attachShadow === 'function' ? highlightedFrameElement.attachShadow({ mode: 'open' }) : undefined)) != null ? left1 : (typeof highlightedFrameElement.createShadowRoot === 'function' ? highlightedFrameElement.createShadowRoot() : undefined)) != null ? left : highlightedFrameElement;
