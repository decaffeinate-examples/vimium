...
const HintCoordinator = {
  ...
  prepareToActivateMode(mode, onExit) {
    ...
    Utils.setTimeout(1000, function() { if (cacheAllKeydownEvents != null ? cacheAllKeydownEvents.modeIsActive : undefined) { return cacheAllKeydownEvents.exit(); } });
    this.onExit = [onExit];
  ...
  activateMode({hintDescriptors, modeIndex, originatingFrameId}) {
    ...
    return DomUtils.documentReady(() => { return Settings.onLoaded(() => {
      ...
      if (Settings.get("filterLinkHints")) { if (this.cacheAllKeydownEvents != null) {
...
var LocalHints = {
  ...
  getLocalHints(requireHref) {
    ...
    while ((visibleElement = visibleElements.pop())) {
      ...
      for (let verticalCoordinate of Array.from(verticalCoordinates)) {
        for (let horizontalCoordinate of Array.from(horizontalCoordinates)) {
